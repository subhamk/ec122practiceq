...
> This is in beta version. If you spot any errors or have any feedback, please do share them with me.
...
```{r prepare-exsum1, exercise = TRUE}
...
```{r exsum2, exercise = TRUE, exercise.setup="prepare-exsum1"}
# variance
var(mpg)
...
```{r exsum3, exercise = TRUE, exercise.setup="prepare-exsum1"}
range(mpg)
```   
...
```{r exsum4, exercise = TRUE, exercise.setup="prepare-exsum1"}
summary(mpg)
```
...
```{r iris2, exercise = TRUE, exercise.setup="prepare-iris1"}
...
```{r iris3, exercise = TRUE, exercise.setup="prepare-iris1"}
barplot(species.freq)             # barplot of the frequency of species
```

To plot a pie chart, we use the `pie` function.

```{r iris4, exercise = TRUE, exercise.setup="prepare-iris1"}
pie(species.freq)
```
